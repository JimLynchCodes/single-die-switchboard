...
export function getUserGuessFromCommandLine(): number {
  ...
  if (!userGuessInput) {
    userGuessInput = reader
      .question("It is now time to make your prediction: 1, 2, 3, 4, 5, or 6: ")
      .trim()
      .toLowerCase();
  }

  // Validate and convert the input to a boolean (heads = true, tails = false)
  const isValidGuess = +userGuessInput === 1 || +userGuessInput === 2 || +userGuessInput === 3 ||
    +userGuessInput === 4 || +userGuessInput === 5 || +userGuessInput === 6;

  if (!isValidGuess) {
    console.error('Please provide a valid integer 1 through 6.');
