...
export function getUserGuessFromCommandLine(): number {
  ...
  const isValidGuess = +userGuessInput === 1 || +userGuessInput === 2 
    || +userGuessInput === 3 || +userGuessInput === 4 
    || +userGuessInput === 5 || +userGuessInput === 6;

  if (!isValidGuess) {
    console.error('Please provide an integer, 1 - 6');
